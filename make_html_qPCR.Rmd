...
# copies/ul viral suspension spiked in (Spike ID: S13)
all_spike_virus_conc <- tibble(Target = c('BCoV_M', 'BRSV_N'), spike_virus_conc = c(35614.1, 56996.1))
...
read_these_sheets <- c('WW21_601 redo') # sheet name(s) in the raw data file (qPCR data dump) - Separate by comma (,)
title_name <- '601 redo + 625 Rice' # name of the filename for writing presentable data and plot title
...
translate_id_manual <- c('WW/Field', 'Water/Field', 'WW/Lab',  'Water/Lab', 'Solids/Present', 'Solids/Removed', 'Bead-beating/Filter', 'Bead-beating/No filter') %>% setNames(str_c('^', c('A','B','C','D','E','F','G','H'))) 
```
...
plot_mean_sd_jitter(summary_bb_qpcr, bb_qpcr, sample_var = extra_categories, exclude_sample = F, x_var = WWTP) %>% format_logscale_y() %>% print()
```
...
vol_qpcr <- bb_qpcr %>% left_join(volumes_data, by = 'Label_tube') %>% 
  left_join(all_spike_virus_conc, by = 'Target') %>% 
...
```{r summarizing_plot, fig.width= 8.5}

spike_qpcr <- vol_qpcr %>% select(-c(3,4,6,7,8)) %>% 
  mutate_at(c('WWTP', 'FACILITY NAME'), ~str_replace_all(., translate_id_manual)) %>% 
  separate(WWTP, c('WWTP', 'tmp'), sep = '/') %>% 
...
```{r percentreco, fig.width= 8.5}

# Plot the % recoveries
plot_mean_sd_jitter(long_vol_qpcr_summary, long_vol_qpcr_raw, long_format = T,  measure_var = 'Recovery fraction', x_var = WWTP, ylabel = 'Fraction of spike-in recovered') %>% print()

# Choose data with smaller % recoveries - exclude categories with higher %
long_smallpercent <- list(long_vol_qpcr_summary, long_vol_qpcr_raw) %>% map(., ~ filter(., !str_detect(`Sample Name`, 'Bead|NTC|Vaccine'))) %>% setNames(c('summary', 'raw'))

plot_mean_sd_jitter(long_smallpercent$summary, long_smallpercent$raw, long_format = T,  measure_var = 'Recovery fraction', x_var = WWTP, ylabel = 'Fraction of spike-in recovered') %>% print()
...
present_N_data <- presentable_vol_qpcr %>% filter(!str_detect(Facility, "Vaccine|NTC|Blank"), str_detect(Target, "^SARS.*N")) %>%  # select only N1,N2 - remove distracting data
