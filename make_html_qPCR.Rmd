...
# copies/ul viral suspension spiked in (Spike ID: S15)
all_spike_virus_conc <- tibble(Target = c('BCoV_M', 'BRSV_N'), spike_virus_conc = c(12318.9, NA))
...
read_these_sheets <- c('dd.WW4_629_N1-N2', 'dd.WW5_630_N1-N2', 'WW25_629_BCoV', 'WW26_629 part2_BCoV') # sheet name(s) in the raw data file (qPCR data dump) - Separate by comma (,)
title_name <- '629 Rice' # name of the filename for writing presentable data and plot title
...
{
  plot_mean_sd_jitter(long_vol_qpcr_summary, long_vol_qpcr_raw, sample_var = str_c(extra_categories, '|Vaccine'), exclude_sample = T, long_format = T,  measure_var = 'Recovery fraction', x_var = WWTP, ylabel = 'Percentage of spike-in recovered') %>% print()
 
  # reordering by % recovery
  WWTP_order_2 <- long_vol_qpcr_summary %>% filter( str_detect(Measurement,'Recovery fraction')) %>% arrange(`mean`) %>% pull(WWTP) %>% as.character() # order samples by ascending order of BCoV
  
  long_vol_qpcr_summary$WWTP %<>% fct_relevel(levels = WWTP_order_2) 
  
  # Excluding water and NTC - plotting without facets
  plot_mean_sd_jitter(long_vol_qpcr_summary, long_vol_qpcr_raw, sample_var = str_c(extra_categories, '|Vaccine|NTC|Water'), exclude_sample = T, long_format = T,  measure_var = 'Recovery fraction', x_var = WWTP, facet_var = NULL, ylabel = 'Percentage of spike-in recovered') %>% print()
