...
```{r Data_input}
processed_data_1 <- read_sheet('https://docs.google.com/spreadsheets/d/19oRiRcRVS23W3HqRKjhMutJKC2lFOpNK8aNUkC-No-s/edit#gid=1386677460', sheet = 'RT-qPCR results', range = 'C2:R119') %>% 
  separate(`Tube ID`, into = c('Tube ID', 'Biological_replicate')) %>% mutate(`Sample Name` = str_replace(`Sample Name`, 'WW', 'HA')) %>% arrange(Target, `Sample Name`) # merge biological replicates

# # bring the sample names list(s)
# tube_lookup <- read_sheet('https://docs.google.com/spreadsheets/d/1mJcCt1wMiOuBic6sRlBZJf8KSNu2y-B5PjzCUu7jPM8/edit#gid=1935582781', sheet = 'RNA extracts', range = 'A1:D83') %>% rename('Tube ID' = `Label on tube`, 'Biobot_ID' = 2, 'Sample Name' = 4) %>% mutate(Metadata = as.character(Metadata), `Tube ID` = as.character(`Tube ID`)) %>% fill(`Sample Name`)

biobot_lookup <- map_df(c('Week 5 (5/11)', 'Week 6 (5/18)') , ~ read_sheet('https://docs.google.com/spreadsheets/d/1ghb_GjTS4yMFbzb65NskAlm-2Gb5M4SNYi4FHE4YVyI/edit#gid=233791008', sheet = .x, range = 'D:F')) %>% rename('Biobot ID' = `BioBot ID#`, 'Symbol' = SYMBOL) %>% mutate('Biobot ID' = str_remove(`Biobot ID`,'\\.'), Symbol = as.character(Symbol))

proc_data <- left_join(processed_data_1, biobot_lookup, by = 'Biobot ID') %>% mutate(Symbol = if_else(str_detect(Symbol, '^X'), `Tube ID`, Symbol))

# write_sheet(proc_data,'https://docs.google.com/spreadsheets/d/1ouk-kCJHERRhOMNP07lXfiC3aGB4wtWXpnYf5-b2CI4/edit#gid=0', sheet = '(Recoveries) Baylor 5/12; 5/19') # save results to a google sheet
```
...
plt1 <- proc_summary %>% ggplot(aes(x = Symbol, y = `Copy #_mean`, colour = Target)) +
  geom_point(size = 2) + geom_errorbar(aes(ymin = `Copy #_mean` - `Copy #_sd`, ymax = `Copy #_mean` + `Copy #_sd`, width = .1)) +
  geom_jitter(data = proc_data, aes(x = Symbol, y = `Copy #`), size = 1, colour = 'black', alpha = .2, width = .2 ) + 
...
# plot_mean_sd_jitter(concise_summary, concise_raw, 'Copy #', colour_var = Target) -> ptst
```
...
```{r comparision_plot}
plt2 <- proc_summary %>% ggplot(aes(x = Symbol, y = `Recovered_mean`, colour = Target)) +
  ...
  geom_jitter(data = proc_data, aes(x = Symbol, y = Recovered), size = 1, colour = 'black', alpha = .2, width = .2 ) + 
  geom_point(data = proc_data, aes(x = Symbol, y = `Actual spike-in`), size = 1, shape = 21, colour = 'black') +
  facet_grid(~`Sample Name`, scales = 'free_x', space = 'free_x') +
...
plt3 <- concise_summary %>% filter(Measurement == 'Recovery fraction') %>% ggplot(aes(x = Symbol, y = mean, colour = Target)) +
  geom_point(size = 2) + geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd, width = errorbar_width)) +
  geom_jitter(data = proc_data, aes(x = Symbol, y = `Recovery fraction`), size = 1, colour = 'black', alpha = .2, width = .2 ) +
  facet_grid(~`Sample Name`, scales = 'free_x', space = 'free_x') +
...
plt2s <- proc_summary %>% filter(str_detect(`Sample Name`,'5/|HA')) %>% ggplot(aes(x = Symbol, y = `Recovered_mean`, colour = Target)) +
  geom_point(size = 2) + geom_errorbar(aes(ymin = `Recovered_mean` - `Recovered_sd`, ymax = `Recovered_mean` + `Recovered_sd`, width = errorbar_width)) +
  geom_jitter(data = proc_data_sel, aes(x = Symbol, y = Recovered), size = 1, colour = 'black', alpha = .2, width = .2 ) + 
  geom_point(data = proc_data_sel, aes(x = Symbol, y = `Actual spike-in`), size = 1, shape = 21, colour = 'black') +
  facet_grid(~`Sample Name`, scales = 'free_x', space = 'free_x') +
  ggtitle('Baylor-Spike-in vaccine quantification') + ylab('Genome copies/l WW')
...
plt3s <- concise_summary %>% filter(Measurement == 'Recovery fraction', str_detect(`Sample Name`,'5/|HA')) %>% ggplot(aes(x = Symbol, y = mean, colour = Target)) +
  geom_point(size = 2) + geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd, width = errorbar_width)) +
  geom_jitter(data = proc_data_sel, aes(x = Symbol, y = `Recovery fraction`), size = 1, colour = 'black', alpha = .2, width = .2 ) +
  facet_grid(~`Sample Name`, scales = 'free_x', space = 'free_x') +
  ggtitle('Baylor-Spike-in vaccine quantification') + ylab('Recovery %')
...
plt4 <- concise_summary %>% filter(Measurement == 'Recovery fraction', str_detect(`Sample Name`, '5/12')) %>% ggplot(aes(x = Symbol, y = mean, colour = Target)) +
  geom_point(size = 2) + geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd, width = errorbar_width)) +
  facet_grid(~`Sample Name`, scales = 'free_x', space = 'free_x') +
  ggtitle('Baylor-Spike-in vaccine quantification') + ylab('Recovery %')
...
plt5 <- concise_summary %>% filter(Measurement == 'Recovery fraction', str_detect(`Sample Name`, '5/1.$')) %>% ggplot(aes(x = Symbol, y = mean, colour = Target)) +
  geom_point(size = 2) + geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd, width = errorbar_width)) +
  geom_jitter(data = filter(proc_data,  str_detect(`Sample Name`, '5/1.$')), aes(x = Symbol, y = `Recovery fraction`), size = 1, colour = 'black', alpha = .2, width = .2 ) +
  facet_grid(~`Sample Name`, scales = 'free_x', space = 'free_x') +
  ggtitle('Baylor-Spike-in vaccine quantification') + ylab('Recovery %')
...
plt6 <- concise_summary %>% filter(Measurement == 'Recovery fraction', str_detect(`Sample Name`, '5/19')) %>% ggplot(aes(x = Symbol, y = mean, colour = Target)) +
  geom_point(size = 2) + geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd, width = errorbar_width)) +
  geom_jitter(data = filter(proc_data,  str_detect(`Sample Name`, '5/19')), aes(x = Symbol, y = `Recovery fraction`), size = 1, colour = 'black', alpha = .2, width = .2 ) +
  facet_grid(~`Sample Name`, scales = 'free_x', space = 'free_x') +
  ggtitle('Baylor-Spike-in vaccine quantification') + ylab('Recovery %')
...
plt7 <- concise_summary %>% mutate(variation = (sd/mean)* 100) %>% filter(`Sample Name` == '5/19', Measurement == 'Recovered') %>% ggplot(aes(x = Symbol, y = variation)) +
  geom_point(size = 2) +
  facet_grid(~`Sample Name`, scales = 'free_x', space = 'free_x') +
