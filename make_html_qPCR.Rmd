...
proc_data %<>% separate(`Tube ID`, into = c('Tube ID', 'Biological_replicate')) %>% arrange(Target, `Sample Name`) # merge biological replicates
...
# plt2 <- concise_summary %>% ggplot(aes(x = `Tube ID`, y = mean, colour = Target)) +
...
Vac, Water and WW => HA filtration (stadler lab)
