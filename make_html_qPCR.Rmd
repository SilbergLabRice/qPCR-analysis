...
all_spike_virus_conc <- tibble(Target = c('BCoV_M', 'BRSV_N'), spike_virus_conc = c(18341.6, 56996.1))
...
presentable_vol_qpcr <- vol_qpcr %>% rename('Facility' = `FACILITY NAME`) %>%
  ...
  mutate_cond(str_detect(Target, '^N|^SARS'), `Recovery fraction` = NA, `Actual spike-in` = NA) %>%
...
present_N_data <- presentable_vol_qpcr %>% filter(!str_detect(Facility, "Vaccine|NTC|Blank|Std"), str_detect(Target, "^SARS")) %>%  # select only N1,N2 - remove distracting data
