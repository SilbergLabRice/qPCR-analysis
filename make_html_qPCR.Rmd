...
spike_virus_conc <- 35614.1
all_spike_virus_conc <- tibble(Target = c('BCoV_M', 'BRSV_N'), c(35614.1, 56996.1) )
...
```{r percentreco, fig.width= 6}

long_smallpercent <- list(long_vol_qpcr_summary, long_vol_qpcr_raw) %>% map(., ~ filter(., !str_detect(`Sample Name`, 'Bead|NTC'))) %>% setNames(c('summary', 'raw'))
