...
library(knitr)
knitr::opts_chunk$set(echo = F, message = F, warning = F, fig.width = 14, fig.height = 4)
...
spike_virus_conc <- (5430.3) * 5/4 # copies/ul viral suspension spiked in (Spike ID: S12; correction included)
...
read_these_sheets <- c('WW17-615_N1-N2', 'WW19_615_BCov')
...
extra_categories = 'TR|Std'
target_order <- c('BCoV', 'N1_multiplex', 'N2_multiplex')
...
presentable_vol_qpcr <- vol_qpcr %>% rename('Facility' = `FACILITY NAME`) %>% 
  ...
  arrange(Facility, biological_replicates, Target) %>% 
