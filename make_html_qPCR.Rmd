...
all_spike_virus_conc <- tibble(Target = c('BCoV_M', 'BRSV_N'), spike_virus_conc = c(3806.7, 56996.1))
...
read_these_sheets <- c('WW27_Test_pMMoV') # sheet name(s) in the raw data file (qPCR data dump) - Separate by comma (,)
title_name <- 'WW27_Test pMMoV' # name of the filename for writing presentable data and plot title
...
plt_week1 <- plot_mean_sd_jitter(summary_bb_qpcr, bb_qpcr, sample_var = extra_categories, exclude_sample = T, x_var = WWTP, facet_var = NULL, colour_var = `Sample Name`)

plt_week1 %>% print()
plt_week1 %>% format_logscale_y() %>% print()
```
...
plt_week1 <- plot_mean_sd_jitter(summary_bb_qpcr, bb_qpcr, sample_var = extra_categories, exclude_sample = T, x_var = `Sample Name`, facet_var = WWTP, colour_var = NULL) + geom_line(aes(group = Target))
...
if(spike_qpcr %>% plyr::empty() %>% !.) {
  ...
  reco_plot <- plot_mean_sd_jitter(long_vol_qpcr_summary, long_vol_qpcr_raw, long_format = T, measure_var = 'Recovered', sample_var = str_c(extra_categories, '|Vaccine'), exclude_sample = T, x_var = WWTP, ylabel = 'Genome copies/l Wastewater') 
...
long_targets_vol <- vol_qpcr %>% 
  select(c(Target, `Sample Name`, biological_replicates:WWTP )) %>% 
...
presentable_vol_qpcr <- vol_qpcr %>% 
  ...
  rename('Copies/ul RNA' = `Copy #`, 'Copies/l WW' = Recovered, 'Spiked-in Copies/l WW' = `Actual spike-in`, 'Date' = `Sample Name`) %>%
  
  # Adding new variables, modifying existing variables
  mutate(Lab = 'R', 'Volume Filtered' = 50, Bottle = NA) %>% 
  mutate_at('Date', ~as.character(.)) %>%
  mutate_at('Facility', ~if_else(. == assay_variable, str_c(Date, '/', assay_variable), .)) %>%
  ...
  select(Facility, WWTP, Date, Lab, `Target Name`, `Original Sample Volume`, `Volume Filtered`, Ct, `Copies/ul RNA`, `Copies/l WW`, Bottle, `Spiked-in Copies/l WW`, `Recovery fraction`) %>%
