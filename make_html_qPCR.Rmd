...
spike_virus_conc <- 22123 # copies/ul viral suspension spiked in (Spike ID: S12)
...
read_these_sheets <- c('WW18-TR_N1-N2')
title_name <- 'Test concentration flow1'

# Biobot ID sheet
bb_sheets <- c('Week 10 (6/15)')

# Extra categories for plotting separately (separate by | like this 'Vaccine|Troubleshooting')
extra_categories = 'TR'
translate_id_manual <- c('Water/Field', 'Water/Lab', 'WW/Field', 'WW/Lab', 'Solids/Present', 'Solids/Removed') %>% rep(each = 3) %>% setNames(str_c('^', 1:18)) %>% rev()
...
```{r summarizing_plot, fig.width=8}
spike_qpcr <- vol_qpcr %>% select(-c(3,4,6,7,8)) %>% 
  ...
  mutate_cond(`Sample Name` == 'TR', 'Sample Name' = WWTP, 'WWTP' = tmp) %>% 
  select(-tmp)
...
reco_plot <- plot_mean_sd_jitter(long_vol_qpcr_summary, long_vol_qpcr_raw, sample_var = 'Vaccine', exclude_sample = T, long_format = T, measure_var = 'Recovered', x_var = WWTP, ylabel = 'Genome copies/l Wastewater') 
